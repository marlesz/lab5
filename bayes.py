...
def loadCsv(filename):
    ...
    lines = f.readlines()
    f.close()
    dataset = [[] for line in lines]
    for nr, line in enumerate(lines):
        fields = line.strip().split(',')
        for i in fields:
            dataset[nr].append(float(i))
    return dataset
...
def summarize(dataset):
    ...
    del summaries[-1] #ostatnia kolumna to numer klasy
    return summaries
...
def calculateClassProbabilities(summaries, inputVector):
    ...
    for classValue, classSummaries in summaries.iteritems():
        for i in range(len(classSummaries)):
            ...
            iv = inputVector[i]
            probabilities[classValue] = calculateProbability(iv, mean, stdev)
